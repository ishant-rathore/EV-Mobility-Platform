...
export function computeBatteryState(profile: EvProfile): BatteryState {
  ...
  const usableCapacityKwh = profile.batteryCapacityKwh * (profile.batteryHealthPercent / 100);
  const availableEnergyKwh = usableCapacityKwh * (profile.currentSocPercent / 100);
  const reserveEnergyKwh = usableCapacityKwh * (profile.reserveSocPercent / 100);
  ...
  return {
    ...
    estimatedRangeKm: Math.floor((availableEnergyKwh * WH_PER_KWH) / profile.efficiencyWhPerKm),
    rangeToReserveKm: Math.floor((usableAboveReserveKwh * WH_PER_KWH) / profile.efficiencyWhPerKm),
  };
}
