...
Module 1. All routes are under `/api/v1/ev`. See [Battery_Model.md](../04-ev-intelligence/Battery_Model.md) for the maths.
...
`backend/src/modules/ev/ev.service.ts` exports `toEnergyContext(summary)` that produces this object directly from an `EvProfileSummary`.

The shared repository in `backend/src/modules/ev/ev.store.ts` is consumed by both the EV routes and
