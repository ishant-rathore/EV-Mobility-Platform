...
Module 1 (EV Profile & Battery Engine) turns a stored vehicle profile into the energy figures every other module relies on. Implemented in [backend/src/modules/ev/ev.service.ts](../../backend/src/modules/ev/ev.service.ts).
...
This exact fixture is the first case in [ev.service.test.ts](../../backend/src/modules/ev/ev.service.test.ts) and the example in [docs/07-api/EV.md](../07-api/EV.md).
