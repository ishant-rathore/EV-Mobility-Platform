...
Implemented P0 modules are EV profile/battery (1), routing/energy (2), traffic prediction (3), traffic diversification (4), charging-station intelligence (5), charger reliability (6), and the recommendation orchestrator (8). `POST /api/v1/journeys/evaluate` is the authoritative workflow: it uses the server-owned EV profile, selects a diversified route, ranks compatible and reachable chargers, returns a reliability-aware primary plus backup, and composes one explainable Module 8 recommendation. Module 7 telemetry already feeds reliability through shared REST/MQTT ingestion and remains the device-monitoring workstream.
